...
ZeroMQTrackDataPublisher::ZeroMQTrackDataPublisher(
    ...
    , total_published_(0)                          // Yayınlanan toplam mesaj sayısı = 0
    , failed_publications_(0)                      // Başarısız yayın sayısı = 0
    , active_connections_(0)                       // Aktif bağlantı sayısı = 0
    , total_latency_ms_(0.0) {                     // Toplam gecikme süresi = 0.0 ms
...
bool ZeroMQTrackDataPublisher::publishDelayCalcTrackData(const domain::model::DelayCalcTrackData& data) {
    ...
    try {
        ...
        if (serialized.empty()) {
            // ++ = increment operator, atomic olarak değeri 1 artır
...
// PUBLIC METHOD - Publisher istatistiklerini döndür
// PublisherStats = nested struct (class içinde tanımlanmış)
...
ZeroMQTrackDataPublisher::PublisherStats ZeroMQTrackDataPublisher::getPublisherStats() const {
    ...
    // Local variable, performans için atomic read'i bir kez yap
    auto total = total_published_.load();
    
    // Ternary operator: condition ? true_value : false_value
    // > operatörü = büyüktür karşılaştırması
    // / operatörü = bölme işlemi (double / size_t = double)
...
void ZeroMQTrackDataPublisher::publishTask(const PublishTask& task) {
    ...
    try {
        ...
        // Performans ölçümü: işlem ne kadar sürdü
        auto end_time = std::chrono::steady_clock::now();
        
        // std::chrono::duration_cast<T> = duration type conversion
        // std::chrono::microseconds = mikrosaniye cinsinden süre
        // .count() = numeric value'yu al
        // / 1000.0 = mikrosaniyeyi milisaniyeye çevir (double division)
        auto latency = std::chrono::duration_cast<std::chrono::microseconds>(end_time - start_time).count() / 1000.0;
        
        // Toplam gecikme süresine ekle (atomic operation)
        // .load() + latency = mevcut değeri oku, latency ekle, geri yaz
...
std::vector<uint8_t> ZeroMQTrackDataPublisher::serializeDelayCalcTrackData(const domain::model::DelayCalcTrackData& data) {
    ...
    try {
        ...
        oss 
            << data.getId() << '|'                    // ID field
            << data.getVelocity() << '|'              // Velocity field
            << data.getPosition() << '|'              // Position field
            << data.getTimeField() << '|'             // Time field
