 ...
 * Usage:
 * @code
 * // Initialize once at startup (static function approach - c_hexagon style)
 * utils::Logger::init("a_hexagon");
 * 
 * // Or with file output
 * utils::Logger::initWithFile("a_hexagon", "logs/app.log");
 * 
 * // Use macros for logging
 * LOG_INFO("Application started");
 * LOG_DEBUG("Processing item {}", itemId);
 ...
 * @details Static class with lazy initialization
...
public:
     ...
     */
    static void init(const std::string& app_name = "a_hexagon",
    ...
    static void logLatency(const std::string& component,
                           ...
                           int64_t latency_us) {
        spdlog::info("[{}] {} latency: {} us", component, operation, latency_us);
    }

    /**
     * @brief Log track data reception
     * @param track_id Track identifier
     * @param hop_latency Latency in microseconds
     */
    static void logTrackReceived(int32_t track_id, int64_t hop_latency) {
        spdlog::debug("Track {} received - Latency: {} us", track_id, hop_latency);
    }

    /**
     * @brief Log track data extrapolation
     * @param track_id Track identifier
     * @param extrap_count Number of extrapolated samples
     * @param latency_us Processing latency in microseconds
     */
    static void logTrackExtrapolated(int32_t track_id, int extrap_count, int64_t latency_us) {
        spdlog::debug("Track {} extrapolated ({} samples) - Latency: {} us",
                     track_id, extrap_count, latency_us);
    }
...
private:
    ...
    static inline bool initialized_{false};  // MISRA: Brace initialization
};
...
// Legacy compatibility - Logger singleton wrapper
class Logger {
public:
    ...
    void initialize(const std::string& name = "a_hexagon",
                    const std::string& logFile = "",
                    [[maybe_unused]] bool async = true) {
        if (logFile.empty()) {
            utils::Logger::init(name);
        } else {
            utils::Logger::initWithFile(name, logFile);
        }
    }
    ...
    [[nodiscard]] bool isInitialized() const noexcept {
        return utils::Logger::isInitialized();
    }

    template<typename... Args>
    void info(spdlog::format_string_t<Args...> fmt, Args&&... args) {
        utils::Logger::info(fmt, std::forward<Args>(args)...);
    }

    template<typename... Args>
    void debug(spdlog::format_string_t<Args...> fmt, Args&&... args) {
        utils::Logger::debug(fmt, std::forward<Args>(args)...);
    }

    template<typename... Args>
    void warn(spdlog::format_string_t<Args...> fmt, Args&&... args) {
        utils::Logger::warn(fmt, std::forward<Args>(args)...);
    }

    template<typename... Args>
    void error(spdlog::format_string_t<Args...> fmt, Args&&... args) {
        utils::Logger::error(fmt, std::forward<Args>(args)...);
    }

    template<typename... Args>
    void critical(spdlog::format_string_t<Args...> fmt, Args&&... args) {
        utils::Logger::critical(fmt, std::forward<Args>(args)...);
    }

    template<typename... Args>
    void trace(spdlog::format_string_t<Args...> fmt, Args&&... args) {
        utils::Logger::trace(fmt, std::forward<Args>(args)...);
    }
